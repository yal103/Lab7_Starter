describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    */
    for (let i = 1; i < prodItemsData.length; i++) {
        ...
        const firstVal = prodItemsData[i];
        if (firstVal.title.length == 0) { allArePopulated = false; }
        if (firstVal.price.length == 0) { allArePopulated = false; }
        if (firstVal.image.length == 0) { allArePopulated = false; }
    }
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    // click first item again, to reset
